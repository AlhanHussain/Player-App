...
export default class Player extends Component {
  ...
  fetch = ()=>{
    axios.get(`https://mern-backend-hyev.onrender.com/players_data/${this.state.id}`)
    // axios.get(`http://localhost:6010/players_data/${this.state.id}`)
    .then(res=>{
      this.setState({
        data:[res.data[0]]
      },()=>{
    ...
    return (
      <div className='text-center pt-5'>
        ...
        {this.state.data? this.state.data.map(res=>(
          ...
          <img  src={res?.imageUrl} className="card-img-top" alt="..."/>
          <div className="card-body">
            <h5 className="card-title">{res?.name}</h5>
            <p className="card-text">{res?.role}</p>
            <p className="card-title">{res?.SR}</p>
            <p className="card-text">{res?.Av}</p>
            <p className="card-text">{res?.country}</p>
            <p className="card-text">{res?.iplteam}</p>
          </div>
        </div>
        )):
        alert("No data ")
